...
var storySettingsEventType = event.Type{Type: "com.yourbridge.story_settings", Class: event.StateEventType}
...
func makeStoryStateKey(ghostMXID id.UserID, messageID string) string {
	return fmt.Sprintf("%s|%s", ghostMXID, messageID)
}
...
func buildStoryStateKey(networkid.UserID) string {
	// Story settings are per-room, so we always use the empty state key.
	return ""
}
...
func (m *MetaConnector) getStorySetting(ctx context.Context, portal *bridgev2.Portal, ghostMXID id.UserID) (bool, error) {
	...
	evt, err := mx.GetStateEvent(ctx, portal.MXID, storySettingsEventType, buildStoryStateKey(networkid.UserID(ghostMXID)))
	if err != nil {
	...
	return decodeStorySetting(evt)
}

func decodeStorySetting(evt *event.Event) (bool, error) {
...
func (m *MetaConnector) setStorySetting(ctx context.Context, portal *bridgev2.Portal, ghostMXID id.UserID, enabled bool) error {
	...
	_, err := m.Bridge.Bot.SendState(ctx, portal.MXID, storySettingsEventType, buildStoryStateKey(networkid.UserID(ghostMXID)), content, time.Time{})
	return err
}
