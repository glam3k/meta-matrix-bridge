...
func (ig *InstagramMethods) SendStoryReply(ctx context.Context, threadID, mediaID, reelID, text string) (string, error) {
	...
	if err != nil {
		...
		evt.Msg("Instagram story reply request errored")
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		evt := ig.client.Logger.Error().
			...
			Str("media_id", mediaID).
			Str("reel_id", reelID)
		evt.Msg("Instagram story reply request failed")
	...
	if err = json.Unmarshal(body, &result); err != nil {
		ig.client.Logger.Error().Err(err).Msg("Failed to decode story reply response")
		return "", fmt.Errorf("failed to decode story reply response: %w", err)
	}
	if strings.ToLower(result.Status) != "ok" {
		return "", fmt.Errorf("story reply rejected with status %s", result.Status)
	}
	if result.Payload.ItemID == "" {
		return "", fmt.Errorf("story reply missing item id in response")
	}
...
func (ig *InstagramMethods) CreateOrGetThread(ctx context.Context, userIDs []string) (string, error) {
	...
	if err != nil {
		...
		evt.Msg("Instagram create thread request errored")
	...
	if resp.StatusCode < 200 || resp.StatusCode >= 300 {
		evt := ig.client.Logger.Error().
			Int("status_code", resp.StatusCode).
			Strs("recipient_users", userIDs)
		evt.Msg("Instagram create thread request failed")
	...
	if strings.ToLower(result.Status) != "ok" {
		return "", fmt.Errorf("create thread rejected with status %s", result.Status)
	}
	...
	if threadID == "" {
		return "", fmt.Errorf("create thread missing thread id in response")
	}
	return threadID, nil
}
